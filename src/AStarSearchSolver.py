...
def solve(benchmarkfile,solution_savepath):
    # Getting Net Info
    benchmark_name = os.path.splitext(os.path.basename(benchmarkfile))[0]
    ...
    plt.savefig(os.path.join(solution_savepath, f'RoutingVisualize_{benchmark_name}.jpg'))
...
#    plt.show()
    ...
    plt.savefig(os.path.join(solution_savepath, f'RoutingVisualize_2D_{benchmark_name}.jpg'))
    # plt.savefig('{path}RoutingVisualize_2D_{benchmark}.jpg'.format(path=solution_savepath,\
    #             benchmark=benchmarkfile))
    plt.close()

    # dump solution for Astar
    result_file = os.path.join(solution_savepath, f'{benchmark_name}_Astar_solution.txt')
...
#    edge_traffic =
    ...
    #benchmarkfile = "./input/test.gr"

    # Test original file
    benchmarkfile1 = "./data/test_benchmark_1.gr"

    # Test converted file
    benchmarkfile2 = "./data/converted_netlist.gr"

    solution_savepath = './MyOutput/solution'
    os.makedirs(solution_savepath, exist_ok=True)

    # Test both files
    print("=== Testing original benchmark file ===")
    try:
        solve(benchmarkfile1, solution_savepath)
        print("Original file test succeeded!")
    except Exception as e:
        print(f"Original file test failed: {e}")

    print("\n=== Testing converted file ===")
    try:
        solve(benchmarkfile2, solution_savepath)
        print("Converted file test succeeded!")
    except Exception as e:
        print(f"Converted file test failed: {e}")
        import traceback
        traceback.print_exc()
