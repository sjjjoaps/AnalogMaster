...
class VisualGraph(object):
    ...
    def show_grid(self):
        ...
        for net_info in self.gridParameters['netInfo']:
            for i in range(4, len(net_info)):
                pinCoord = net_info[str(i - 3)]
                axes[pinCoord[2]].plot(pinCoord[0], pinCoord[1], 'x')
        plt.show()
        return
...
if __name__ == '__main__':
    filename = 'benchmark/test_benchmark_15.gr'
...
#
    gridParameters = gridParameters(grid_info)
